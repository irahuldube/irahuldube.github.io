...
(function () {
  ...
  const EM_ICON   = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>`;
  ...
  async function init() {
    try {
      const [main, pf] = await Promise.all([
        ...
        fetch('data/portfolio-v2-data.json',{ cache: 'no-store' }).then(r => r.json())
      ]);
      populate(main, pf);
    } catch (e) {
  ...
  function initCanvas() {
    ...
    let W, H, pts;
    const COUNT   = 55;
    const PALETTE = [[14,165,233],[6,182,212],[59,130,246],[99,102,241],[186,230,253]];
